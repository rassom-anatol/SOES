...
**1 ms is confirmed viable.** A full cycle at 25 MHz has a p99 of 220 us, leaving roughly 780 us of headroom in a 1 ms period, so the 2 ms fallback is not needed on present evidence. What remains unmeasured is SYNC0 edge wakeup latency under PREEMPT_RT, which adds to this figure and is what `cyclictest` on the real carrier (§3.6) must establish before `SyncErrorCounterLimit` can be sized (§3.3.1).
...
**The ESI is named after the device, not `slave.xml`.** It is the one generated artifact that leaves this repository — copied onto removable media and into a master's device description folder — and two devices in this tree once both had an ESI called `slave.xml`, differing in identity, mailbox size and SyncManager layout. Confusing them means flashing the wrong SII, which is the single step in deployment with no undo. The master shows the device by name, so the file should too.
...
**The SDO-Info walk completed.** TwinCAT issued 4 Get OD List, 33 Get Object Description and 73 Get Entry Description requests plus an individual upload of every object, and the slave answered all of them from OP with no abort and no AL status code. That exercises `MBXSIZE 256` end to end and is the verification for the string objects in [`stack-review.md`](stack-review.md) §2.5.

**A trap worth recording, because it cost most of a session.** TwinCAT caches the CoE object list per slave, and a cached empty result survives Update List, Auto Update and a change of slave state — it kept serving the empty list from before the device description was corrected. Toggling the source setting in the CoE-Online Advanced dialog away from and back to *Online - via SDO Information* forces a genuine re-query. Until that toggle, the master sent no SDO-Info at all, which from the slave side is indistinguishable from a device the master believes has no CoE support. **When a master shows an empty object list, establish whether it is actually asking before changing anything in the slave** — a debug build logs every CoE service that arrives ([`esc_coe.c`](../soes/esc_coe.c), `ESC_coeprocess`), and that distinction is not visible any other way.
